...
export default {
  ...
  computed: {
    byIdData() {
      return this.$store.state.liveListInProgress.byIdData;
    },
    showEdit: {
      get: function() {
        return this.$store.state.liveListInProgress.showEdit;
      },
      set: function(val) {
        this.$store.commit("liveListInProgress/changeShowEdit", val);
      }
    },
    popupLoading() {
      return this.$store.state.liveListInProgress.popupLoading;
    }
  },
  mounted() {},
  methods: {
    open(id) {
      ...
      if (this.id) {
        this.$store.dispatch({
          type: "liveListInProgress/getByIdData",
          id: this.id
        });
      } else {
        this.$store.commit({ type: "liveListInProgress/isNewData" });
      }
      this.$store.commit("liveListInProgress/changeShowEdit", true);
    },
    result() {
    ...
    onOk() {
      ...
      this.$refs["editForm"].validate(valid => {
        if (valid) {
          this.$store.dispatch({
            type: "liveListInProgress/saveData",
            id: this.id
          });
        }
      });
    ...
    onCancel() {
      this.$store.commit("liveListInProgress/changeShowEdit", false);
    },
    visibleChange(val) {
      this.$store.commit("liveListInProgress/changePageWait", val);
    }
  }
};
</script>
