...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["businessQueueList/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["businessQueueList/getByIdData"](option))
      .then(res => {
  ...
  sendInfo({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["businessQueueList/sendInfo"](option))
      .then(res => {
        if (res && res.data.code == 200) {
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["businessQueueList/delete"](option))
      .then(res => {
