...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["messageRecord/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["messageRecord/getByIdData"](option))
      .then(res => {
        _this
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[option.id ? "messageRecord/saveEdit" : "messageRecord/saveNew"](
        option
      )
    )
      .then(res => {
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["messageRecord/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["messageRecord/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["messageRecord/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["messageRecord/delete"](option))
      .then(res => {
        _this
          .dispatch("login/errorLogin", res.data)
          .then(type => {
