...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["textAndPictureEditing/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["textAndPictureEditing/getByIdData"](option))
      .then(res => {
        _this
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[option.id ? "textAndPictureEditing/saveEdit" : "textAndPictureEditing/saveNew"](
        option
      )
    )
      .then(res => {
        if (res.data.code == 1000) {
          ...
          router.push("/textAndPictureEditing");
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["textAndPictureEditing/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["textAndPictureEditing/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["textAndPictureEditing/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["textAndPictureEditing/delete"](option))
      .then(res => {
        _this
          .dispatch("login/errorLogin", res.data)
          .then(type => {
