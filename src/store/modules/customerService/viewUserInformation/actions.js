...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["viewUserInformation/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["viewUserInformation/getByIdData"](option))
      .then(res => {
        _this
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[option.id ? "viewUserInformation/saveEdit" : "viewUserInformation/saveNew"](
        option
      )
    )
      .then(res => {
        if (res.data.code == 1000) {
          ...
          router.push("/viewUserInformation");
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["viewUserInformation/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["viewUserInformation/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["viewUserInformation/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["viewUserInformation/delete"](option))
      .then(res => {
        _this
          .dispatch("login/errorLogin", res.data)
          .then(type => {
