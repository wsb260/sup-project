...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["interfaceManagement/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["interfaceManagement/getByIdData"](option))
      .then(res => {
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["interfaceManagement/delete"](option))
      .then(res => {
