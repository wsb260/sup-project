...
const apiInfo = api.liveBroadcastManagement.rebroadcastList;
export default {
  ...
  getGiftData: (state, getters, rootState, rootGetters) => option => {
    var ajaxData = {
      pageNo: option && option.current ? option.current : state.giftCurrent || 1,
      pageSize:
        option && option.page_size ? option.page_size : state.giftPage_size || 10
    };
    if (state.giftSearchType) {
      var searchData = {};
      forEachToJson(state.giftSearchData, (key, v) => {
        if (Boolean(v) && typeof v === "object") {
          searchData[key] = timeText(v);
        } else if (Boolean(v) && typeof v === "string") {
          searchData[key] = v;
    ...
    return {
      method: apiInfo.getGiftData.method,
      url:
        apiInfo.getGiftData.url +
        (makeGetAjaxUrl(searchData) != "?" ? makeGetAjaxUrl(searchData) : "") +
        (makeGetAjaxUrl(searchData) != "?"
          ? "&" + makeAjaxUrl(ajaxData)
          : "?" + makeAjaxUrl(ajaxData))
