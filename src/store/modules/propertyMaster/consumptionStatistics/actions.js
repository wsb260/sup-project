...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["consumptionStatistics/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["consumptionStatistics/getByIdData"](option))
      .then(res => {
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[
        option.id
          ? "consumptionStatistics/saveEdit"
          : "consumptionStatistics/saveNew"
      ](option)
    )
      .then(res => {
        if (res.data.code == 1000) {
          ...
          router.push("/consumptionStatistics");
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["consumptionStatistics/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["consumptionStatistics/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["consumptionStatistics/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["consumptionStatistics/delete"](option))
      .then(res => {
        if (res.data.code == 1000) {
