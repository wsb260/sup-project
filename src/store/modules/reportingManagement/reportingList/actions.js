...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["reportingList/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["reportingList/getByIdData"](option))
      .then(res => {
  ...
  warning({ commit, state }, option) {
    var _this = this;
    axios(this.getters["reportingList/warning"](option));
    then(res => {
      if (res && res.code == 200) {
        _this.commit("notice/saveSuccess", res.data.message);
      }
    });
  },
  prohibition({ commit, state }, option) {
    var _this = this;
    axios(this.getters["reportingList/prohibition"](option));
    then(res => {
      if (res && res.code == 200) {
        _this.commit("notice/saveSuccess", res.data.message);
      }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["reportingList/delete"](option))
      .then(res => {
        _this
