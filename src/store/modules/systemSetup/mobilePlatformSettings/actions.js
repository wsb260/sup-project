...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["mobilePlatformSettings/getData"]())
      .then(res => {
        if (res && res.data.code == 1000 && res.data.data.mobilePlatformSettingss != null) {
          option.data = res.data.data.mobilePlatformSettingss;
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["mobilePlatformSettings/getByIdData"](option))
      .then(res => {
        _this
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[option.id ? "mobilePlatformSettings/saveEdit" : "mobilePlatformSettings/saveNew"](
        option
      )
    )
      .then(res => {
        if (res.data.code == 1000) {
          ...
          router.push("/mobilePlatformSettings");
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["mobilePlatformSettings/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["mobilePlatformSettings/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["mobilePlatformSettings/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["mobilePlatformSettings/delete"](option))
      .then(res => {
        _this
          .dispatch("login/errorLogin", res.data)
          .then(type => {
