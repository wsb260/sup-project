...
export default {
  ...
  getData({ dispatch, commit, state }, option) {
    ...
    axios(this.getters["invitationManagement/getData"]())
      .then(res => {
  ...
  getByIdData({ dispatch, commit, state }, option) {
    var _this = this;
    commit("isNewData");
    axios(this.getters["invitationManagement/getByIdData"](option))
      .then(res => {
        _this
  ...
  saveData({ dispatch, commit, state }, option) {
    ...
    axios(
      this.getters[option.id ? "invitationManagement/saveEdit" : "invitationManagement/saveNew"](
        option
      )
    )
      .then(res => {
        if (res.data.code == 1000) {
          ...
          router.push("/invitationManagement");
  ...
  sendUserList({ dispatch, commit, state }, option) {
    axios(this.getters["invitationManagement/sendUserList"](option))
      .then(res => {
        if (res.data.code == 1000) {
          option.data = res.data;
  ...
  updateFileState({ dispatch, commit, state }, option) {
    axios(this.getters["invitationManagement/updateFileState"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  upLoad({ dispatch, commit, state }, option) {
    axios(this.getters["invitationManagement/upLoad"](option))
      .then(res => {
        if (res.data.code == 1000) {
          dispatch({ type: "refreshData" });
        }
  ...
  delete({ dispatch, commit, state }, option) {
    var _this = this;
    axios(this.getters["invitationManagement/delete"](option))
      .then(res => {
        _this
          .dispatch("login/errorLogin", res.data)
          .then(type => {
